...
void USBCDC::_onRX(){
    ...
    if (count) {
        //ESP_LOGE(TAG, "write rx_queue [%d]: %.*s", count, count, buf);
        // NOTE: when received data consider as connected
        if(!connected){
            connected = true;
            arduino_usb_cdc_event_data_t p;
            arduino_usb_event_post(ARDUINO_USB_CDC_EVENTS, ARDUINO_USB_CDC_CONNECTED_EVENT, &p, sizeof(arduino_usb_cdc_event_data_t), portMAX_DELAY);
...
{
    // NOTE: check DTR bit for connected state
    // Most but not all terminal client set this when making connection
    if(itf >= MAX_USB_CDC_DEVICES || tx_lock == NULL || (!tud_cdc_n_connected(itf)  && !connected)){
        return;
    }
...
{   
    // NOTE: check DTR bit for connected state
    // Most but not all terminal client set this when making connection
    if(itf >= MAX_USB_CDC_DEVICES || tx_lock == NULL || buffer == NULL || size == 0 || (!tud_cdc_n_connected(itf)  && !connected)){
        return 0;
    }
