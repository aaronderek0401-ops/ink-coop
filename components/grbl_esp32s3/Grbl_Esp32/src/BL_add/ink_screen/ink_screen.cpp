...
// ======== 子母数组系统变量 ========
bool g_in_sub_array = false;  // 是否在子数组模式（非static，供json_layout.cpp使用）
...
// ==================== JSON布局全局变量 ====================
// 注意：JSON布局全局变量已移至 json_layout.cpp/json_layout.h
// 通过 #include "json_layout.h" 引入

// ==================== JSON函数前置声明 ====================
// 注意：JSON布局函数已移至 json_layout.cpp/json_layout.h
// 通过 #include "json_layout.h" 引入
...
// ================== 单词本文本缓存（PSRAM）==================
// 注意：单词本文本缓存的变量和函数已移至 word_book.cpp/word_book.h
// 这里仅保留必要的外部引用（通过 #include "word_book.h" 引入）
...
const TextArrayEntry g_text_arrays[] = {
    ...
    {"wordbook_wrong_translation", "$wordbook_idx", g_wordbook_wrong_translation_ptrs, WORDBOOK_CACHE_COUNT}, // 错误翻译（测试用）
    {"wordbook_option_1", "$wordbook_idx", g_wordbook_option1_ptrs, WORDBOOK_CACHE_COUNT},  // 选项1（随机）
    {"wordbook_option_2", "$wordbook_idx", g_wordbook_option2_ptrs, WORDBOOK_CACHE_COUNT},  // 选项2（随机）
...
/**
 * @brief 左侧任务区翻页（只翻左侧，矩形1-3）
 */
void nextDecordTaskPage() {
    // 切换页码（0 <-> 1）
    g_decord_task_page_offset = (g_decord_task_page_offset == 0) ? 1 : 0;
    
    ESP_LOGI("DECORD", "🔄 左侧任务区翻页到第%d页", g_decord_task_page_offset);
    
    // 更新任务区（矩形1-3）的文本索引：$becord_task_idx_0/1/2
    for (int i = 0; i < 3; i++) {
        char var_name[48];
        int new_index = g_decord_task_page_offset * 3;  // 第0页→0, 第1页→3
        
        snprintf(var_name, sizeof(var_name), "$becord_task_idx_%d", i);
        for (int j = 0; j < g_text_arrays_count; j++) {
            if (strcmp(g_text_arrays[j].var_name, var_name) == 0) {
                g_text_animation_indices[j] = new_index;
                ESP_LOGI("DECORD", "  更新任务文本 %s: %d", var_name, new_index);
                break;
            }
        }
        
        snprintf(var_name, sizeof(var_name), "$becord_task_timeremain_idx_%d", i);
        for (int j = 0; j < g_text_arrays_count; j++) {
            if (strcmp(g_text_arrays[j].var_name, var_name) == 0) {
                g_text_animation_indices[j] = new_index;
                break;
    ...
    if (g_json_rects && g_json_rect_count > 0) {
        redrawJsonLayout();
        ESP_LOGI("DECORD", "左侧任务区已刷新显示第%d页内容", g_decord_task_page_offset);
    }
}

/**
 * @brief 右侧生活区翻页（只翻右侧，矩形4-6）
 */
void nextDecordLifePage() {
    // 切换页码（0 <-> 1）
    g_decord_life_page_offset = (g_decord_life_page_offset == 0) ? 1 : 0;
    
    ESP_LOGI("DECORD", "🔄 右侧生活区翻页到第%d页", g_decord_life_page_offset);
    
    // 更新生活区（矩形4-6）的文本索引：$becord_life_idx_3/4/5
    for (int i = 0; i < 3; i++) {
        char var_name[48];
        int new_index = g_decord_life_page_offset * 3;  // 第0页→0, 第1页→3
        
        snprintf(var_name, sizeof(var_name), "$becord_life_idx_%d", i + 3);
        for (int j = 0; j < g_text_arrays_count; j++) {
            if (strcmp(g_text_arrays[j].var_name, var_name) == 0) {
                g_text_animation_indices[j] = new_index;
                ESP_LOGI("DECORD", "  更新生活文本 %s: %d", var_name, new_index);
                break;
            }
        }
        
        snprintf(var_name, sizeof(var_name), "$becord_life_timeremain_idx_%d", i + 3);
        for (int j = 0; j < g_text_arrays_count; j++) {
            if (strcmp(g_text_arrays[j].var_name, var_name) == 0) {
                g_text_animation_indices[j] = new_index;
                break;
        ...
        // 更新图标的索引
        snprintf(var_name, sizeof(var_name), "$becord_life_status_idx_%d", i + 3);
        for (int j = 0; j < g_icon_arrays_count; j++) {
            if (strcmp(g_icon_arrays[j].var_name, var_name) == 0) {
                g_animation_indices[j] = new_index;
                break;
    ...
    if (g_json_rects && g_json_rect_count > 0) {
        redrawJsonLayout();
        ESP_LOGI("DECORD", "右侧生活区已刷新显示第%d页内容", g_decord_life_page_offset);
    }
}
...
void displayMainScreen(RectInfo *rects, int rect_count, int status_rect_index, int show_border) {
    ...
    do {
        ...
        for (int i = 0; i < rect_count; i++) {
            ...
            // ==================== 绘制顺序说明 ====================
            // 1. 先绘制静态图标 (icons) - 底层
            // 2. 再绘制动态图标 (icon_roll) - 中层，会覆盖静态图标
            // 3. 最后绘制文本 (text_roll) - 顶层，会覆盖图标
            // 这样可以确保动态元素始终在静态元素上方
            // ===================================================
            ...
            // 显示该矩形内的所有动态图标组（icon_roll，中层，会覆盖静态图标）
            if (rect->icon_roll_count > 0) {
                ...
                for (int j = 0; j < rect->icon_roll_count; j++) {
                    ...
                    // 获取当前帧索引（用于定位轨迹点）
                    int frame_idx = getVariableIndex(icon_roll->idx);
                    if (frame_idx < 0) frame_idx = 0;
                    
                    // 根据轨迹点数量循环获取位置
                    int path_idx = (icon_roll->path_count > 0) ? (frame_idx % icon_roll->path_count) : 0;
                    float current_rel_x = icon_roll->rel_x[path_idx];
                    float current_rel_y = icon_roll->rel_y[path_idx];
                    
                    ESP_LOGI("MAIN", "  处理动态图标组%d: arr=%s, idx=%s, icon=%d, path[%d/%d]=(%.3f,%.3f)", 
                            j, icon_roll->icon_arr, icon_roll->idx, current_icon_index,
                            path_idx, icon_roll->path_count, current_rel_x, current_rel_y);
                    
                    if (current_icon_index >= 0 && current_icon_index < ICON_CACHE_COUNT) {
                        // 计算图标位置（基于左上角对齐）
                        int icon_x = rect->x + (int)(current_rel_x * rect->width);
                        int icon_y = rect->y + (int)(current_rel_y * rect->height);
            ...
            // 显示该矩形内的所有动态文本组（text_roll，顶层，会覆盖图标）
