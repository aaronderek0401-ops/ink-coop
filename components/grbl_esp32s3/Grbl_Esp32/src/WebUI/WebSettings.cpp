...
#    endif
    ...
    static Error deleteBatchSDObject(char* parameter, AuthenticationLevel auth_level) {
        parameter = trim(parameter);
        if (*parameter == '\0') {
            webPrintln("Missing index pattern!");
            return Error::InvalidValue;
        }

        webPrintln("parameter: " + String(parameter));
        ...
        DIR *dir = opendir("/sd");
        if (dir == NULL) {
            webPrintln("Failed to open root directory!");
            return Error::FsFailedDelDir;
        }
        char* tempParameter = strdup(parameter);
        char* indexStr = strtok(tempParameter, ",");
        uint16_t cnt = 0;
        uint16_t deleteCnt = 0;
        struct dirent *entry;
        bool flag = false;
        while (indexStr != nullptr) {
            int index = atoi(indexStr); 
           
            while ((entry = readdir(dir)) != NULL && cnt < index) {
                char path[312];
                snprintf(path, sizeof(path), "%s/%s", "/sd", entry->d_name);
            ...
            if (flag) {
                webPrintln("File index out of range: " + String(index));
            }else {
                ...
                file2del.close();
                if (!SD.remove(path)) {
                    webPrintln("Cannot delete file: " + String(path));
                } else {
                     webPrintln("File deleted: " + String(path));
                    cnt++;
                    deleteCnt++;
                }
            }
            indexStr = strtok(nullptr, ","); 
        }
        closedir(dir);
        free(tempParameter);
        if(deleteCnt==0) {
            webPrintln("batch delete failed");
        }else {
            grbl_msg_sendf(CLIENT_ALL, MsgLevel::Info, "batch delete completed,delete flie:%d",deleteCnt);
        }
        return Error::Ok;
    }
    static Error deleteAllSDFiles(char* parameter, AuthenticationLevel auth_level) {  // ESP217
        // parameter = trim(parameter);
        
        // if (parameter && strcasecmp(parameter, "confirm") != 0) {
        //     webPrintln("Warning: This will delete ALL FILES (not directories) in SD card!");
        //     webPrintln("Use 'deleteAllSDFiles=confirm' to proceed");
        //     return Error::InvalidValue;
        // }
        ...
        ESP_LOGI("DELETE_FILES", "Starting to delete all files (excluding directories)");
        
        DIR *dir = opendir("/sd");
        if (dir == NULL) {
            webPrintln("Failed to open SD card directory!");
            return Error::FsFailedDelDir;
        }
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            String full_path = "/sd/" + String(entry->d_name);
            
            struct stat entry_stat;
            if (stat(full_path.c_str(), &entry_stat) == 0) {
                if (!S_ISDIR(entry_stat.st_mode)) {
                    // 只删除文件，不删除目录
                    ESP_LOGI("DELETE_FILES", "Deleting file: %s", full_path.c_str());
                    ...
                    if (SD.remove(full_path.c_str())) {
                        ...
                        if (remove(full_path.c_str()) == 0) {
                            ...
                            failCount++;
                            ESP_LOGE("DELETE_FILES", "Failed to delete: %s", full_path.c_str());
                        }
                    }
                }
            }
        ...
        webPrintln("File deletion completed: " + String(successCount) + " deleted, " + String(failCount) + " failed");
        grbl_msg_sendf(CLIENT_ALL, MsgLevel::Info, "SD files cleanup: %d deleted", successCount);
    ...
    static Error inkScreenTestTwo(char* parameter, AuthenticationLevel auth_level) {  // ESP210
        if(parameter != nullptr) {
            int value = atoi(parameter);
            // 使用转换后的值
...
#ifdef ENABLE_WIFI
        static Error StaBegin(char* parameter, AuthenticationLevel auth_level)
        {  
...
#define WEB_COMMON
    ...
    void make_web_settings() {
        ...
        new WebCommand("file_or_directory_path", WEBCMD, WU, "ESP215", "SD/BatchDelete", deleteBatchSDObject);
...
#ifdef ENABLE_WIFI
        new WebCommand(NULL, WEBCMD, WU, NULL, "WiFi/Begin", StaBegin);
