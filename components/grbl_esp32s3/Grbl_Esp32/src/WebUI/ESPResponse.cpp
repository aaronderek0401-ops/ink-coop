...
namespace WebUI {
    ...
    ESPResponseStream::ESPResponseStream(uint8_t client, httpd_req_t *req) {
        _header_sent = false;
        _webserver   = NULL;
        _client      = CLIENT_WEBUI;
        _req         = req;
    ...
    void ESPResponseStream::print(const char* data) {
        ...
        if (_req) {
            if (!_header_sent) {
                httpd_resp_set_hdr(_req, "Content-Type", "text/html");
                httpd_resp_set_hdr(_req, "Cache-Control", "no-cache");
                httpd_resp_set_status(_req, HTTPD_200);
                _header_sent = true;
            }
            _buffer += data;
            if (_buffer.length() > 1200) {
                //send data
                httpd_resp_send_chunk(_req, _buffer.c_str(), _buffer.length());
                //reset buffer
                _buffer = "";
            }
            return;
