...
namespace WebUI {
    ...
    String WiFiConfig::setAPSSID(void) {
        uint8_t mac[6];
        char    ssid[22] = { 0 };
        ...
        sprintf(ssid, "BLGS_%02X%02X", mac[4], mac[5]);
        WebUI::wifi_ap_ssid->setStringValue(ssid);
        return String(ssid);
    }
    ...
    const char* WiFiConfig::info() {
        ...
        if ((WiFi.getMode() == WIFI_MODE_AP) || (WiFi.getMode() == WIFI_MODE_APSTA)) {
            ...
            tmp =  wifi_ap_password ? wifi_ap_password->get() : "Null";
            result += tmp;
        }
    ...
    bool WiFiConfig::begin(bool flag) {
        //stop active services
        wifi_services.end();
        //setup events
        if (!_events_registered) {
