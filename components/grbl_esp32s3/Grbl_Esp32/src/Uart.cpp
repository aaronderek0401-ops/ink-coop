...
Uart::Uart(int uart_num, size_t rx_buffer_size, size_t tx_buffer_size, uint8_t queue_num, QueueHandle_t *queue_handle) : 
_uart_num(uart_port_t(uart_num)), 
_pushback(-1),
_queue_num(queue_num),
_queue_handle(queue_handle),
_rxBufferSize(rx_buffer_size),
_txBufferSize(tx_buffer_size)
{
...
void Uart::begin(unsigned long baudrate, Data dataBits, Stop stopBits, Parity parity) {
    ...
    uart_driver_install(_uart_num, _rxBufferSize, _txBufferSize, _queue_num, _queue_handle, ESP_INTR_FLAG_IRAM);
}
